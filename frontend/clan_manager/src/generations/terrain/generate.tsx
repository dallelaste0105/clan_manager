...
import { saveArchipelagoAndHisIslands } from "../../connections/archipelagos";
...
export async function GenerateTerrain(
  ...
  seed3: number,
  seed4: number,
  shouldSave: boolean // <--- NOVO PARAMETRO: define se deve salvar ou só desenhar
) {
  const width = 1280;
  const height = 720;
  const matrix: boolean[][] = [];
  const islandsList: number[] = [];
  ...
  // Função auxiliar para salvar somente se necessário
  const handleSave = async (list: number[]) => {
      if (shouldSave) {
          await saveArchipelagoAndHisIslands(list);
      }
  };
  ...
  if (islandQuantityDeterminant === 0 || islandQuantityDeterminant === 1) {
    if (firstPixelDeterminant <= 3) {
      ...
      islandsList[0] = seed1;
      await handleSave(islandsList);
      GenerateCity(ctx, matrix, 640 - s1[4] * 20, 640 + s1[4] * 20, 360 - s1[4] * 20, 360 + s1[4] * 20, s1, 0);
    } else if (firstPixelDeterminant <= 6) {
      createIsland(450, 360, s1);
      islandsList[0] = seed1;
      await handleSave(islandsList);
      GenerateCity(ctx, matrix, 450 - s1[4] * 20, 450 + s1[4] * 20, 360 - s1[4] * 20, 360 + s1[4] * 20, s1, 0);
    } else {
      createIsland(830, 360, s1);
      islandsList[0] = seed1;
      await handleSave(islandsList);
      GenerateCity(ctx, matrix, 830 - s1[4] * 20, 830 + s1[4] * 20, 360 - s1[4] * 20, 360 + s1[4] * 20, s1, 0);
    }
    ...
    GenerateCity(ctx, matrix, 320 - (s1[4] * 20)/2, 320 + (s1[4] * 20)/2, 360 - (s1[4] * 20)/2, 360 + (s1[4] * 20)/2, s1, 0);
    GenerateCity(ctx, matrix, 960 - s2[4] * 20, 960 + s2[4] * 20, 360 - s2[4] * 20, 360 + s2[4] * 20, s2, 0);
    ...
    GenerateCity(ctx, matrix, 640 - s1[4] * 20, 640 + s1[4] * 20, 180 - s1[4] * 20, 180 + s1[4] * 20, s1, 0);
    GenerateCity(ctx, matrix, 320 - s2[4] * 20, 320 + s2[4] * 20, 540 - s2[4] * 20, 540 + s2[4] * 20, s2, 0);
    GenerateCity(ctx, matrix, 960 - s3[4] * 20, 960 + s3[4] * 20, 540 - s3[4] * 20, 540 + s3[4] * 20, s3, 0);

    
  } else {
    createIsland(320, 200, s1); createIsland(960, 200, s2); createIsland(320, 520, s3);
    ...
    GenerateCity(ctx, matrix, 320 - s1[4] * 20, 320 + s1[4] * 20, 200 - s1[4] * 20, 200 + s1[4] * 20, s1, 0);
    GenerateCity(ctx, matrix, 960 - s2[4] * 20, 960 + s2[4] * 20, 200 - s2[4] * 20, 200 + s2[4] * 20, s2, 0);
    GenerateCity(ctx, matrix, 320 - s3[4] * 20, 320 + s3[4] * 20, 520 - s3[4] * 20, 520 + s3[4] * 20, s3, 0);
  }
}